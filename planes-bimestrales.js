...
function planCount(group) {
  if (group.kind === 'trunk') return 4;
  if (group.kind === 'laboratory' || group.kind === 'workshop') return 2;
  if (group.kind === 'other') return group.duration === 'annual' ? 4 : 2;
  return 2;
}
...
function blankPlan(group, number) {
  return {
    number,
    name: group.kind === 'trunk' ? '' : `${group.name} ${number}`,
    synopsis: '',
    contentIds: [],
    objectives: '',
...
function persist() {
  ...
  window.app = current;
  window.dispatchEvent(new CustomEvent('pci-state-change', { detail: { source: 'planes-bimestrales' } }));
}

function planStatus(plan) {
  const hasText = Boolean(plan.name || plan.synopsis || plan.objectives || plan.contentIds?.length);
  const stageValues = STAGES.map(([id]) => plan.stages?.[id] ?? {});
  const hasStageWork = stageValues.some((stage) => stage.description || stage.duration || stage.resources || stage.activities);
  const allActivities = stageValues.every((stage) => String(stage.activities ?? '').trim());
...
function ensureStyles() {
  if (document.getElementById('pciPlansStyles')) return;
  const style = document.createElement('style');
  style.id = 'pciPlansStyles';
  style.textContent = `
...
function contentRows(group, plan) {
  ...
  if (!available.length) return '<div class="pci-plan-note">Este espacio todavía no tiene contenidos asignados en el PCI. Primero asigná contenidos al espacio y después vas a poder seleccionarlos para cada plan.</div>';
...
function stageEditor(stageId, label, stage) {
  return `<details class="pci-stage" open>
    ...
    <div class="pci-stage-body">
      <label class="pci-plan-field full"><span>¿Qué va a pasar en esta etapa? (opcional)</span><small>Encabezado breve dirigido a los y las estudiantes.</small><textarea data-stage="${stageId}" data-stage-field="description" placeholder="Ej.: En esta etapa vas a recuperar ideas previas y anticipar el problema que vamos a trabajar...">${escapeHtml(stage.description)}</textarea></label>
      <label class="pci-plan-field"><span>Duración estimada de la etapa</span><input data-stage="${stageId}" data-stage-field="duration" value="${escapeHtml(stage.duration)}" placeholder="Ej.: 2 clases"></label>
      <label class="pci-plan-field full"><span>Recursos</span><textarea data-stage="${stageId}" data-stage-field="resources" placeholder="Materiales, textos, videos, herramientas, espacios...">${escapeHtml(stage.resources)}</textarea></label>
      <label class="pci-plan-field full"><span>Actividades dirigidas al estudiante</span><small>Acá se escribe detalladamente qué tiene que hacer el estudiante.</small><textarea class="activities" data-stage="${stageId}" data-stage-field="activities" placeholder="1. Observá...\n2. Conversá con tu grupo...\n3. Registrá...">${escapeHtml(stage.activities)}</textarea></label>
    </div>
  </details>`;
}
...
function renderPlanList(area, group) {
  ...
  shell.innerHTML = `
    ...
    <div class="pci-plan-body">
      <div class="pci-plan-note">${group.kind === 'trunk' ? 'Este nivel tiene 4 planes bimestrales.' : group.kind === 'laboratory' || group.kind === 'workshop' ? 'Este laboratorio/taller tiene 2 planes bimestrales.' : `Este espacio tiene ${plans.length} planes bimestrales según su duración.`}</div>
      <div class="pci-plan-grid" style="margin-top:14px">${plans.map((plan) => `<article class="pci-plan-card"><span class="pci-plan-status">${escapeHtml(planStatus(plan))}</span><h3 style="margin-top:10px">Plan ${plan.number}</h3><p style="margin:0;color:var(--muted,#6a7b84)">${escapeHtml(plan.name || (group.kind === 'trunk' ? `Bimestre ${plan.number}` : `${group.name} ${plan.number}`))}</p><button class="button primary" type="button" data-open-plan="${plan.number}">Abrir plan</button></article>`).join('')}</div>
    </div>`;
  ...
  shell.innerHTML = `
    ...
    <div class="pci-plan-body"><form id="pciPlanForm" class="pci-plan-form">
      <section class="pci-plan-section"><h3>Datos generales</h3><div class="pci-plan-fields">
        ...
        <label class="pci-plan-field full"><span>Nombre del plan</span><small>${group.kind === 'trunk' ? 'En los espacios troncales se especifica un nombre para el plan.' : `Para este tipo de espacio puede consignarse “${escapeHtml(group.name)} ${number}”.`}</small><input data-plan-field="name" value="${escapeHtml(plan.name)}" placeholder="${escapeHtml(group.kind === 'trunk' ? `Nombre del plan · bimestre ${number}` : `${group.name} ${number}`)}"></label>
        <label class="pci-plan-field"><span>Duración</span><div class="pci-plan-readonly">1 bimestre</div></label>
        <label class="pci-plan-field"><span>Ubicación temporal del plan</span><div class="pci-plan-readonly">Bimestre ${number} de ${plans.length}</div></label>
        <label class="pci-plan-field full"><span>Sinopsis</span><small>Breve presentación de lo que trata este plan.</small><textarea data-plan-field="synopsis" placeholder="Contá en menos de un párrafo de qué trata este plan...">${escapeHtml(plan.synopsis)}</textarea></label>
      </div></section>

      <section class="pci-plan-section"><h3>Contenidos</h3><p class="pci-plan-note">Seleccioná entre los contenidos que ya fueron asignados a este espacio en el PCI. Podés acotar su alcance en la redacción del plan.</p>${contentRows(group, plan)}</section>

      <section class="pci-plan-section"><h3>Objetivos</h3><label class="pci-plan-field full"><span>Se espera que logres...</span><small>Aprendizajes que las y los estudiantes deberán alcanzar. Se recomienda redactarlos en infinitivo.</small><textarea data-plan-field="objectives" placeholder="Analizar...\nReconocer...\nProducir...">${escapeHtml(plan.objectives)}</textarea></label></section>

      <section class="pci-plan-section"><h3>Etapas del plan</h3><p class="pci-plan-note">En cada etapa, la descripción de qué va a pasar es opcional. El bloque de actividades está pensado para escribir las consignas detalladas directamente para el estudiante.</p><div style="display:grid;gap:12px;margin-top:12px">${STAGES.map(([id, label]) => stageEditor(id, label, plan.stages[id])).join('')}</div></section>

      <div class="pci-plan-actions"><button class="button secondary" type="button" data-back-plans>← Volver a los planes</button><button class="button primary" type="submit">Guardar plan</button></div>
    </form></div>`;

  shell.querySelector('[data-plans-close]')?.addEventListener('click', closePlans);
  shell.querySelector('[data-back-plans]')?.addEventListener('click', () => { saveCurrentEditor(group, plan); renderPlanList(area, group); });
  shell.querySelector('#pciPlanForm')?.addEventListener('submit', (event) => { event.preventDefault(); saveCurrentEditor(group, plan); renderPlanList(area, group); });
}

function saveCurrentEditor(group, plan) {
  const form = document.getElementById('pciPlanForm');
  if (!form) return;
  form.querySelectorAll('[data-plan-field]').forEach((field) => { plan[field.dataset.planField] = field.value; });
  ...
  form.querySelectorAll('[data-stage][data-stage-field]').forEach((field) => {
    const stage = plan.stages[field.dataset.stage] ??= blankStage();
...
function addGroupEntryPoints() {
  document.querySelectorAll('.group-card[data-group-id]').forEach((card) => {
    ...
    const plans = Array.isArray(found.group.plansBimestrales) ? found.group.plansBimestrales : [];
    ...
    entry.innerHTML = `<div><strong>Planes bimestrales</strong><small>${count} plan${count === 1 ? '' : 'es'} · ${started} iniciado${started === 1 ? '' : 's'}</small></div><button class="button accent" type="button">Abrir planes</button>`;
    entry.querySelector('button').addEventListener('click', (event) => { event.stopPropagation(); openPlans(card.dataset.groupId); });
    const footer = card.querySelector('.group-footer');
    if (footer) card.insertBefore(entry, footer); else card.appendChild(entry);
  });
}

...
function activeTerms(group){
  const start=clampTerm(group.termStart,1);
  const end=Math.max(start,clampTerm(group.termEnd,start));
...
function normalizeHoursByTerm(group){
  ...
  const base=numericHours(group.weeklyHours??group.hours??0);
  const terms=activeTerms(group);
  if(group.hoursMode==='uniform'){
    terms.forEach(term=>{group.hoursByTerm[term]=base});
  }else{
    terms.forEach(term=>{
      if(group.hoursByTerm[term]==null)group.hoursByTerm[term]=base;
      group.hoursByTerm[term]=numericHours(group.hoursByTerm[term]);
    });
  }
  group.weeklyHours=base;
  group.hours=base;
}
...
function normalizeTemporalGroup(area,index,group){
  ...
  group.weeklyHours=numericHours(group.weeklyHours);
  if(isLevelArea(area)){
...
function termHoursEditor(index,group){
  return activeTerms(group).map(term=>`<label>C${term} · HC semanales
    <input data-pci-term-hours="${index}" data-pci-term="${term}" type="number" min="0" step="0.5" value="${numericHours(group.hoursByTerm?.[term]??group.weeklyHours)}">
  </label>`).join('');
}
...
function temporalEditor(area,index,group){
  ...
  return `<div class="pci-temporal" data-temporal="${index}">
    ...
    <div class="pci-temporal-grid">
      ...
      <label>HC predeterminadas
        <input data-pci-hours="${index}" type="number" min="0" step="0.5" value="${group.weeklyHours}">
      </label>
    </div>
    <div class="pci-term-hours-title">Carga efectiva por cuatrimestre</div>
    <div class="pci-term-hours-grid">${termHoursEditor(index,group)}</div>
    <div class="pci-temporal-note">${fixed?`El ${group.name||`Nivel ${group.level}`} es anual y ocupa C${group.termStart} y C${group.termEnd}.`:`${componentLabel} · C${group.termStart}${group.termEnd!==group.termStart?` a C${group.termEnd}`:''}`} · ${group.hoursMode==='per_term'?'Carga diferenciada por cuatrimestre.':'Carga uniforme en todos los cuatrimestres activos.'}</div>
  </div>`;
}
...
function bindTemporalEditors(){
  ...
  document.querySelectorAll('[data-pci-hours]').forEach(el=>el.onchange=()=>updateUniformHours(el,Number(el.value||0)));
  document.querySelectorAll('[data-pci-term-hours]').forEach(el=>el.onchange=()=>updateTermHours(el,Number(el.value||0)));
}

function groupFromElement(element){
  const raw=element.dataset.pciComponent??element.dataset.pciLevel??element.dataset.pciStart??element.dataset.pciEnd??element.dataset.pciHours??element.dataset.pciTermHours;
  const index=Number(raw);
  const area=app.current;
  return {area,index,group:app.areas?.[area]?.groups?.[index]};
}

function updateGroup(element,field,value){
  const {area,index,group}=groupFromElement(element);
  if(!group)return;
  group[field]=value;
...
function updateUniformHours(element,value){
  const {area,index,group}=groupFromElement(element);
  if(!group)return;
  group.hoursMode='uniform';
  group.weeklyHours=numericHours(value);
  normalizeTemporalGroup(area,index,group);
  save(0);
  renderGroups();
}

function updateTermHours(element,value){
  const {area,index,group}=groupFromElement(element);
  if(!group)return;
  const term=clampTerm(element.dataset.pciTerm,group.termStart);
  group.hoursMode='per_term';
  if(!group.hoursByTerm||typeof group.hoursByTerm!=='object')group.hoursByTerm={};
  group.hoursByTerm[term]=numericHours(value);
  normalizeTemporalGroup(area,index,group);
  save(0);
  renderGroups();
}
...
function injectStyles(){
  ...
  style.textContent=`
    ...
    .pci-temporal-grid label,.pci-term-hours-grid label{display:grid;gap:5px;font-size:.76rem;font-weight:850}
    .pci-temporal-grid select,.pci-temporal-grid input,.pci-term-hours-grid input{width:100%;border:1px solid var(--line);border-radius:9px;padding:8px;background:#fbfcfc}
    .pci-term-hours-title{font-weight:900;font-size:.82rem;margin:12px 0 7px}
    .pci-term-hours-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:8px}
  ...
  if(brand&&!brand.querySelector('.pci-schema-badge'))brand.insertAdjacentHTML('beforeend','<span class="pci-schema-badge">Modelo temporal v2</span>');
}

...
function ensureStyles() {
  ...
  style.textContent = `
    .matrix-detail-backdrop{position:fixed;inset:0;z-index:1400;display:grid;place-items:center;padding:20px;background:rgba(15,45,61,.42);backdrop-filter:blur(2px)}
    .matrix-detail-backdrop[hidden]{display:none}
    .matrix-detail-panel{width:min(820px,100%);max-height:calc(100vh - 40px);overflow:auto;margin:0;padding:18px;border:1px solid var(--line,#d6e2e5);border-radius:18px;background:#fff;box-shadow:0 24px 70px rgba(21,55,74,.28);overscroll-behavior:contain}
    ...
    .matrix-detail-head h3{margin:2px 0 0;font-size:1.08rem}
    .matrix-detail-meta{color:var(--muted,#6a7b84);font-size:.8rem;font-weight:800}
    .matrix-detail-actions{display:flex;gap:8px;flex-wrap:wrap}
    .matrix-detail-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin:10px 0}
    .matrix-detail-box{padding:11px;border-radius:12px;background:#f5f9f9}
    .matrix-detail-box strong{display:block;margin-bottom:4px;font-size:.76rem;text-transform:uppercase;letter-spacing:.04em}
    .matrix-detail-box p{margin:0;color:var(--ink-soft,#425c68);line-height:1.45}
    .matrix-content-list{display:grid;gap:8px;margin-top:12px}
    .matrix-content-row{padding:10px 11px;border:1px solid var(--line,#d6e2e5);border-radius:11px;background:#fff}
    .matrix-content-row small{display:block;margin-bottom:4px;color:var(--muted,#6a7b84);font-weight:800}
    .matrix-content-row p{margin:0;line-height:1.42}
    ...
    @media(max-width:820px){.matrix-header{position:static!important;top:auto!important}}
    @media(max-width:720px){.matrix-detail-backdrop{padding:10px;align-items:end}.matrix-detail-panel{width:100%;max-height:88vh;border-radius:18px 18px 10px 10px;padding:15px}.matrix-detail-grid{grid-template-columns:1fr}.matrix-detail-head{display:block}.matrix-detail-actions{margin-top:10px}}
  `;
...
function openDetails(groupId, trigger = null) {
  ...
  const contextual = group.kind === 'integration'
    ? fieldBox('Contexto problematizador', group.context)
    : ['formative', 'technical-workshop'].includes(group.kind)
  ...
  panel.innerHTML = `
    <div class="matrix-detail-head">
      <div><div class="matrix-detail-meta">${escapeHtml(area)} · ${escapeHtml(kindLabel(group))} · ${escapeHtml(temporal)}${group.type ? ` · ${escapeHtml(group.type)}` : ''}</div><h3 id="matrixDetailsTitle">${escapeHtml(group.name)}</h3></div>
...
window.addEventListener('DOMContentLoaded', ensureCreatorCredit);
setTimeout(ensureCreatorCredit, 0);
